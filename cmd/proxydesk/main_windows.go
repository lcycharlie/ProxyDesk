	...
	exitCode, err := MainWindow{
		...
		Children: []Widget{
			Composite{
				...
				Children: []Widget{
					GroupBox{
						...
						Children: []Widget{
							Label{Text: "国家"},
							ComboBox{AssignTo: &countryCB, Model: countries, CurrentIndex: 0},
							Label{Text: "协议"},
							ComboBox{AssignTo: &protocolCB, Model: []string{"HTTP", "SOCKS5"}, CurrentIndex: 0},
							Label{Text: "本地端口"},
							LineEdit{AssignTo: &portEdit, Text: "7890"},
							Label{Text: "上游代理"},
							TextEdit{AssignTo: &upstreamEdit, MinSize: Size{Width: 360, Height: 110}},
						},
					},
					GroupBox{
						Title:  "当前连接",
						...
						Children: []Widget{
							Label{Text: "状态"},
							Label{AssignTo: &statusLabel, Text: "未启动"},
							Label{Text: "出口 IP"},
							Label{AssignTo: &exitIPLabel, Text: "-"},
							Label{Text: "本地端口"},
							Label{AssignTo: &localLabel, Text: "127.0.0.1:7890"},
							Label{Text: "上游代理"},
							Label{AssignTo: &upstreamLabel, Text: "-"},
							Label{Text: "错误"},
							Label{AssignTo: &errorLabel, Text: "-"},
						},
					},
				},
			},
			Composite{
				Layout: HBox{},
				Children: []Widget{
					PushButton{Text: "启动转发", OnClicked: startRoute},
					PushButton{Text: "停止", OnClicked: stopRoute},
					PushButton{Text: "测试出口", OnClicked: testExitIP},
					PushButton{Text: "开启系统代理", OnClicked: enableSystemProxy},
					PushButton{Text: "关闭系统代理", OnClicked: disableSystemProxy},
				},
			},
			GroupBox{
				Title:  "供应商 API",
				...
				Children: []Widget{
					Label{Text: "API 地址"},
					LineEdit{AssignTo: &apiEndpoint},
					Label{Text: "国家参数"},
					LineEdit{AssignTo: &apiCountryParam},
					Label{Text: "JSON 字段"},
					LineEdit{AssignTo: &apiJSONKey},
					HSpacer{},
					PushButton{Text: "按国家获取 IP", OnClicked: fetchAPI},
				},
			},
			GroupBox{
				Title:  "日志",
				Layout: VBox{},
				Children: []Widget{
					TextEdit{AssignTo: &logBox, ReadOnly: true, MinSize: Size{Height: 150}},
				},
			},
		},
	}.Run()
